...
import {authSelector, logout} from "../../stores/features/auth/authSlice";
...
function Main(props: { layout?: "side-menu" | "simple-menu" | "top-menu" }) {
  ...
  const result = capitalizeFirstLetter(formattedPathname);
      ...
      >
        <div className="flex items-center h-full">
          ...
          >
            ...
            <Breadcrumb.Link to={full_url} active={true}>
              {/* Update Profile */}
              {result}
            </Breadcrumb.Link>
          </Breadcrumb>
          ...
          {/* BEGIN: Account Menu */}
          <Menu>
            <Menu.Button className="block w-8 h-8 overflow-hidden rounded-full shadow-lg image-fit zoom-in intro-x" style={{background:"#fff"}}>
              <img
                alt=""
            ...
            <Menu.Items className="w-56 mt-px relative bg-primary/80 before:block before:absolute before:bg-black before:inset-0 before:rounded-md before:z-[-1] text-white">
              <Menu.Header className="font-normal">
                <div className="font-medium">{auth?.userInfo?.companyName}</div>
                <div className="text-xs text-white/70 mt-0.5 dark:text-slate-500">
                  {auth?.userInfo?.userName}
                </div>
              </Menu.Header>
              <Menu.Divider className="bg-white/[0.08]" />
              <Menu.Item className="hover:bg-white/5" onClick={goToProfile}>
                <Lucide icon="User" className="w-4 h-4 mr-2" /> Profile
              </Menu.Item>
              <Menu.Divider className="bg-white/[0.08]" />
              <Menu.Item className="hover:bg-white/5" onClick={logoutApplication}>
